...
const NewSales = () => {
  ...
  const fetchCustomersList = async () => {
    const { data } = await axios.get(`${baseUrl}/client-list`);
    setCustomers(data.data);
  };

  useEffect(() => {
    fetchCustomersList().catch(console.lo);
  }, []);
  ...
  const handleSelectProductChange = (e) => {
    ...
    const index = products.findIndex(
      (value) => value.p_name === selectedProductName
    );
    ...
    setselectedProductDetails((prev) => ({
      ...prev,
      subtotal: (prev.price * prev.qty * ((100 - prev.discount) / 100)).toFixed(
        2
      ),
    }));
  };
  ...
  const handleSelectDiscountChange = (e) => {
    ...
    setselectedProductDetails({
      ...selectedProductDetails,
      discount: Number(selectedProducDisc),
    });
    // update subtotal:
    setselectedProductDetails((prev) => ({
      ...prev,
  ...
  const handleAddToCart = (e) => {
    ...
    const index = cartItems.findIndex(
      (value) => value.c_name === selectedProductDetails.name
    );
    if (index >= 0) {
            ...
            <form onSubmit={handleSubmit(onSubmit)} id='form_new_sales'>
              ...
              <label className='form-control w-full lg:max-w-xs px-2'>
                ...
                <select
                  ...
                  className='select '
                  {...register('clientId', { validate: validateCustName })}
                  defaultValue={0}
                >
              ...
              <div className='form-box flex flex-wrap md:flex-nowrap'>
                ...
                <div className='w-full'>
                  ...
                  <div className='label-text text-xs text-error h-8 p-2'>
                    {errors.cust_contact && (
                      <p>{errors.cust_contact.message}</p>
                    )}
                  </div>
                </div>
              </div>
